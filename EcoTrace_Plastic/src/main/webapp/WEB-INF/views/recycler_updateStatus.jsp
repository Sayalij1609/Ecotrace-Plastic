...
<%@ page import="java.util.List, com.ecotrace.plastic.Model.Collection_Log" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>View Collection Request</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <ul>
                <li >
                    <a href="recyclerDashboard.jsp" class="nav-link">
...
<div class="main-content">
    ...
    <div class="content-container">
        ...
        <table class="recyclers-table">
            ...
            <tbody>
                <% 
                    List<Collection_Log> cols = (List<Collection_Log>) request.getAttribute("cols"); 
                    if (cols != null && !cols.isEmpty()) {
                        for (Collection_Log col : cols) { 
                ...
                <tr>
                    <td><%= col.getId() %></td>
                    <td><%= col.getPhone() %></td>
                    <td><strong><%= col.getRecyclerName()  %></strong></td>
                    <td><%= col.getMaterials() %></td>
                    <td><%= col.getPickupDate() %></td>
                    <td><%= col.getPickupTime() %></td>
                    <td><%= col.getPickup_condition() %></td>
                    <td>
                            <% if ("pending".equalsIgnoreCase(col.getStatus())) { %>
                                <span class="status-badge status-pending">Pending</span>
                            <% } else if ("approved".equalsIgnoreCase(col.getStatus())) { %>
                                <span class="status-badge status-approved">Approved</span>
                            <% }else if ("Rejected".equalsIgnoreCase(col.getStatus())) { %>
                            <span class="status-badge status-approved">Rejected</span>
                            <% } else { %>
                                <span class="status-badge status-rejected">Collected</span>
                            <% } %>
                        </td>
                         <td>
                            <div class="action-btns">
                                <form action="ApproveRequest" method="post" style="display:inline;">
                                    <input type="hidden" name="uid" value="<%=col.getId() %>" />
                                ...
                                <form action="UpdateRequest" method="post" style="display:inline;">
                                    <input type="hidden" name="uid" value="<%= col.getId() %>"  />
                                    <button class="action-btn btn-reject" type="submit">
                                        <i class="fas fa-times-circle"></i> Collected
                                    </button>
                                </form>
                            </div>
                        </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('statusFilter').addEventListener('change', function () {
                ...
                const tr = document.querySelectorAll('.recyclers-table tbody tr');
                ...
                tr.forEach(row => {
                    ...
                    if (!badge || filter === 'all') {
                        ...
                        row.style.display = badge.textContent.toLowerCase().includes(filter) ? '' : 'none';
                    }
                });
            });
